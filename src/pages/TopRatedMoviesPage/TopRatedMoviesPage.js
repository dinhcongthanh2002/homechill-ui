...
import * as topRatedMoviesService from "../../services/topRatedMoviesService";
import Image from "../../components/Image/Image";

const TopRatedMoviesPage = () => {
  const [topRatedMoviesMovies, setTopRatedMovies] = useState([]);
  const [nextPage, setNextPage] = useState(1);
  const handleNextPage = () => {
    const fetchApi = async () => {
      const result = await topRatedMoviesService.getTopRatedMovies(nextPage);
      return setTopRatedMovies(result.results);
    };
    fetchApi();
    ...
    <div className="mt-16 text-white lg:max-w-[90rem] mx-auto px-2 lg:px-4">
      ...
      <div className="mb-3">
        <div className="p-5 bg-[#303b42] text-[15px] leading-[22px]">
          <p className="inline-block text-primary font-bold leading-6">
            Phim Được Xếp Hạng Hàng Đầu
          </p>{" "}
          mới nhất, Tổng hợp danh sách các bộ phim hay được web cập nhật liên
          tục. Tải hơn 10.000 bộ phim được xếp hạng hàng đầu năm{" "}
          {new Date().getFullYear() - 1}, {new Date().getFullYear()} vietsub,
          thuyết minh mới nhất, hay nhất.
        </div>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-5 grid-rows-3 gap-[10px]">
        {topRatedMoviesMovies.length > 0 &&
          topRatedMoviesMovies.map((movie, index) => (
            <Link
              to={`/movie/${movie.id}`}
              key={movie.id}
