...
import * as familyMoviesService from "../../services/familyMoviesService";
import Image from "../../components/Image/Image";

const FamilyMoviesPage = () => {
  const [familyMoviesMovies, setFamilyMovies] = useState([]);
  const [nextPage, setNextPage] = useState(1);
  const handleNextPage = () => {
    const fetchApi = async () => {
      const result = await familyMoviesService.getFamilyMovies(nextPage);
      return setFamilyMovies(result.results);
    };
    fetchApi();
    ...
    <div className="mt-16 text-white lg:max-w-[90rem] mx-auto px-2 lg:px-4">
      ...
      <div className="mb-3">
        <div className="p-5 bg-[#303b42] text-[15px] leading-[22px]">
          <p className="inline-block text-primary font-bold">Phim Gia Đình</p>{" "}
          mới nhất, Tổng hợp danh sách các bộ phim hay được web cập nhật liên
          tục. Tải hơn 10.000 bộ phim gia đình năm{" "}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 grid-rows-3 gap-[10px]">
        {familyMoviesMovies.length > 0 &&
          familyMoviesMovies.map((movie, index) => (
            <Link
              to={`/movie/${movie.id}`}
              key={movie.id}
