...
import * as adventureMoviesService from "../../services/adventureMoviesService";
...
const AdventureMoviesPage = () => {
  ...
  const handleNextPage = () => {
    const fetchApi = async () => {
      const result = await adventureMoviesService.getAdventureMovies(nextPage);
      return setAdventureMovies(result.results);
    };
    fetchApi();
    ...
    <div className="mt-16 text-white lg:max-w-[90rem] mx-auto px-2 lg:px-4">
      ...
      <div className="mb-3">
        <div className="p-5 bg-[#303b42] text-[15px] leading-[22px]">
          <p className="inline-block text-primary font-bold">Phim Phiêu Lưu</p>{" "}
          mới nhất, Tổng hợp danh sách các bộ phim hay được web cập nhật liên
          tục. Tải hơn 10.000 bộ phim phiêu lưu năm{" "}
