...
import * as actionMoviesService from "../../services/actionMoviesService";
...
const ActionMoviesPage = () => {
  ...
  const handleNextPage = () => {
    const fetchApi = async () => {
      const result = await actionMoviesService.getActionMovies(nextPage);
      return setActionMovies(result.results);
    };
    fetchApi();
  ...
  return (
    <div className="mt-16 text-white lg:max-w-[90rem] mx-auto px-2 lg:px-4">
      <h1 className="uppercase lg:text-[25px] text-primary font-bold select-none cursor-text lg:py-4 mt-24 mb-3 text-lg">
        Phim hành động
      </h1>
      <div className="mb-3">
        <div className="p-5 bg-[#303b42] text-[15px] leading-[22px]">
          <p className="inline-block text-primary font-bold">Phim Hành Động</p>{" "}
          mới nhất, tổng hợp danh sách các bộ phim hay được web cập nhật liên
          tục. Tải hơn 10.000 bộ phim hành động năm{" "}
