...
import * as documentaryMoviesService from "../../services/documentaryMoviesService";
...
const DocumentaryMoviesPage = () => {
  ...
  const handleNextPage = () => {
    const fetchApi = async () => {
      const result = await documentaryMoviesService.getDocumentaryMovies(
        nextPage
      );
      return setDocumentaryMovies(result.results);
    };
    fetchApi();
    ...
    <div className="mt-16 text-white lg:max-w-[90rem] mx-auto px-2 lg:px-4">
      ...
      <div className="mb-3">
        <div className="p-5 bg-[#303b42] text-[15px] leading-[22px]">
          <p className="inline-block text-primary font-bold">Phim Tài Liệu</p>{" "}
          mới nhất, Tổng hợp danh sách các bộ phim hay được web cập nhật liên
          tục. Tải hơn 10.000 bộ phim tài liệu năm{" "}
